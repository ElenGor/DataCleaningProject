...
def find_outliers_quantile(data, feature, left=0.01, right=0.99):
    """
    Находит выбросы в данных, используя метод интервала, обозначенного левым и правым квантилем.
    Все наблюдения, не попавшие в интервал, относятся к выбросам. 
    Границы интервала с обоих сторон могут регулироваться вручную.
    Args:
        data (pandas.DataFrame): набор данных
        feature (str): имя признака, на основе которого происходит поиск выбросов
        left (float, optional): уровень квантилей, отсекаемых слева. По умолчанию 0.01.
        right (float, optional): уровень квантилей, отсекаемых справа. По умолчанию 0.99.
